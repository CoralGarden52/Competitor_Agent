...
def test_policy_qa_pass_goes_to_draft() -> None:
    state = _state()
    ...
    assert decision.next_stage == StageName.draft
    assert decision.transition_reason == TransitionReason.qa_passed


def test_policy_qa_fail_collect_rework() -> None:
    ...
    decision = TransitionPolicy.decide(run_state=state, stage=StageName.qa, stage_result=qa, error=None)
    assert decision.next_stage == StageName.collect
    assert decision.transition_reason == TransitionReason.qa_rework_collect
    assert decision.recovery_state == RecoveryState.reworking
    assert decision.apply_rework_ticket is True
