...
本项目以 `PostgreSQL` 作为核心持久化存储，用于保存竞品分析任务在执行过程中的关键数据，包括运行状态、事件日志、结构化证据、字段级分析结果、报告内容、问答记录、问卷结果以及质量返工信息。  
从系统设计角度看，数据库并不仅仅承担“存结果”的作用，更承担了任务回放、过程追踪、报告修订、问卷导出和质量闭环支撑等职责。

本文件给出一份适合软件工程报告和比赛材料使用的核心 ER 图抽象。
